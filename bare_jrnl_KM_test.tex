...
\title{Scheduling method of multi-UAV search system with edge computing}
%
%
...
%However, despite the temporal requirement in the surveillance scenarios, in many prior works, time consumed for data processing and transfer has not been taken into consideration.
...
%; the usefulness of the information that the user finally obtains has not been evaluated.
%Therefore, this paper proposes scheduling method of multi-UAV search system that maximizes the user's utility in consideration of processing time of image data and data transfer time.
...
We also present the problem formulation of the proposed scheduling method that maximizes the user’s utility, which is calculated from the efficiency of obtaining results from analyzed data and the interval of obtaining the results.
%The efficiency of obtaining results from analyzed data and the interval of obtaining the results are used as the user's utility.
%Simulation results verify the effectiveness of the proposed method by evaluating the information that the user finally obtains in terms of two metrics: i) elapsed time from start time and ii) elapsed time after acquired for each information.
...
%The efficiency of collecting data by UAVs (collecting efficiency) and the interval of obtaining results from analyzed data are used as the user's utility.\textcolor{red}{The efficiency of obtaining results from analyzed data (obtaining efficiency) and the interval of obtaining the results (obtaining interval) are used as the user's utility.}Simulation results verify the effectiveness of the proposed method by evaluating the usefulness of the information obtained by the user, which is measured by i) the number of information pieces obtained within a certain period and ii) the freshness of individual obtained piece of information.
\end{abstract}
\begin{IEEEkeywords}
...
\begin{table}[t]
\centering
\caption{the average value of utility}
  \begin{tabular}{|p{2cm}||c|} \hline
method & average value \\ \hline
Proposed & 0.191 \\ \hline
Fixed($N^i$=1) & 0.003  \\ \hline
Fixed($N^i$=5) & 0.031  \\ \hline
Fixed($N^i$=10) & 0.013  \\ \hline
Fixed($N^i$=50) & 0.004  \\ \hline
\end{tabular}
\label{para_val}
\end{table}
...
\begin{enumerate}
  ...
  % 1)
  \item スケジューリング複雑性: スケジューリング対象(UAV，AP，バッテリ)の種類が多くなるほど複雑性が増す．
  \begin{itemize}
    \item $++$: スケジューリング対象が1種類(UAVのみ)
    \item $+$: スケジューリング対象が2種類(UAVとバッテリ，またはUAVとAP)
    \item $-$: スケジューリング対象が3種類(UAV，AP，バッテリ)
  \end{itemize}
  % 2)
  \item 交換機構複雑性: UAVやAPに直接充電する場合は非接触電力伝送も可能である一方，バッテリの着脱が伴う場合は物理的機構が複雑になり障害耐性が低下する．
  \begin{itemize}
    \item $++$: バッテリ交換機構不必要
    \item $+$: 充電ポートがバッテリを交換する場合
    \item $-$: UAVがバッテリを交換する場合
  \end{itemize}
  % 3)
  \item 運搬機構複雑性: UAVがバッテリやAPを運搬する場合は，運搬のための機構および着脱機構が必要である．また，運搬中は積荷の落下や他との衝突，UAVの制御が不安定になる可能性などが考えられる．
  \begin{itemize}
    \item $++$: UAVが荷物を運搬する必要なし
    \item $+$: UAVがバッテリのみを運搬
    \item $-$: UAVがAPを運搬
  \end{itemize}
  % 4)
  \item 必要UAV数: UAVがAPに対して充電を行なっている時や充電ポートから充電されている時は，UAVが長時間停止してしまう分UAVが多く必要である．一方バッテリ交換や充電済みAPを設置する場合は，給電時間は充電する場合と比べて無視できる．
  \begin{itemize}
    \item $++$: バッテリ交換をするか，充電済みAPを設置する場合のみの構成
    \item $+$: UAVまたはAPどちらかがバッテリ交換をするか，充電済みAPを設置する場合
    \item $-$: UAV，APともに充電される場合
  \end{itemize}
  % 5)
  \item 必要AP数: APを充電ポートで充電する場合にはAPが余分に必要となる．
  \begin{itemize}
    \item $+$: AP設置点にAPが設置されており，それ以上APが必要でない場合
    \item $-$: 充電ポートがAPに充電し，UAVが充電済みAPを運搬するモデル
    %\item $(評価なし):$ UAV-AP一体型の場合はUAV数=AP数となるため，必要UAV数を参照のこと
  \end{itemize}
  % 6)
  \item 充電ポートの種類数: 充電対象となるデバイスの種類が多くなるほど，充電ポートに用意しなければならない充電機器の種類も増加し，充電ポートの複雑性が増すばかりでなく，インフラ建設コストも増大する．
  \begin{itemize}
    \item $+$: 1種類(UAVのみ)
    \item $-$: 2種類(UAVとバッテリ，バッテリとAP，またはUAVとAP)
  \end{itemize}
  % 7)
  \item 充電損失: 充電ポートによって充電されたUAVがAPに充電する場合や，充電ポートによって充電されたAPがUAVに充電する場合は，充電損失が発生する．
  \begin{itemize}
    \item $+$: 下記以外の場合
    \item $-$: 充電ポートがUAVに充電し，UAVがAPに充電する場合，または充電ポートがAPに充電し，APがUAVに充電する場合
  \end{itemize}
  % 8)
  \item 構成部品の独立性: 特にUAVやバッテリは近年技術的進歩のスピードが速く，交換時に構成部品が独立に分離可能である方が新技術を導入しやすい．
  \begin{itemize}
    \item $+$: UAV-AP一体型モデル
    \item $-$: UAV-AP分離型モデル
  \end{itemize}
...
method & Total utility vs. Elapsed time from start time (one-dimention) &Elapsed time vs. area id &total value vs. Elapsed time from start time \\ \hline
Proposed & ++ & +  & +\\ \hline
Fixed($N^i$=1) & + & - & - - \\ \hline
Fixed($N^i$=4) & + & -  & -\\ \hline
Fixed($N^i$=8) & - & ++ & + \\ \hline
...
%\caption{the average value of utility}
  \begin{tabular}{|p{1.5cm}||p{2cm}|p{2cm}|p{2cm}|} \hline
method & total image number vs. $\frac{P_e}{\overline{P_u}}$ & Total image number vs. $U_M$ & Total image number vs. $D$\\ \hline
Proposed & ++ & ++ & ++ \\ \hline
Fixed($N^i$=1) & - - & - - & - - \\ \hline
Fixed($N^i$=4) & - - & - -  & -\\ \hline
Fixed($N^i$=8) & - & -  & -\\ \hline
